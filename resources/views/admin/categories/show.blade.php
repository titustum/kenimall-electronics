<x-layouts.app> {{-- Assuming your admin layout component is x-layouts.app --}}
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl p-4">
        ...
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-4">Category Details: {{ $category->name }}</h2>

        <div class="bg-white dark:bg-neutral-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Category Name --}}
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Category Name</p>
                    <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">{{ $category->name }}</p>
                </div>

                {{-- Category ID --}}
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Category ID</p>
                    <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">{{ $category->id }}</p>
                </div>

                {{-- Slug --}}
                <div class="md:col-span-2">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Slug</p>
                    <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">{{ $category->slug }}</p>
                </div>

                {{-- Category Image --}}
                <div class="md:col-span-2">
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Category Image</p>
                    ...
                    @else
                    <p class="text-gray-600 dark:text-gray-400">No image uploaded for this category.</p>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
                        {{ $category->addedBy->name ?? 'N/A' }} {{-- Assuming 'addedBy' relationship exists --}}
                    </p>
                </div>
